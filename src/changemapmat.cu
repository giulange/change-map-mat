...
*/
__global__ void changemap_old( const uint8_T *iMap1,const uint8_T *iMap2,
                           ...
                           uint32_T *chMat, uint32_T *oMap )
{
	...
	if (tid<ntilesX*ntilesY){
        ...
        */
		i = tiledimX * ( ntilesX*tiledimY*(tid/ntilesX) + tid%ntilesX );// ***pitch!!
                           ...
                           uint32_T *chMat, uint32_T *oMap )
{
	...
	if (tid<nY){
        ...
        */
		// The pointer along the x direction is straightforward:
		for( x=0; x<nX; x++ ){
			oMap[i_row+x] = ( (uint32_T)(iMap1[i_row+x]) + (uint32_T)(iMap2[i_row+x])*100 );
			chMat[k+((uint32_T)(iMap2[i_row+x]))+((uint32_T)(iMap1[i_row+x]))*nclasses]++;
		}
	}
}
...
//	unsigned int		deltaX=0,deltaY=0;
	...
	// allocate on RAM
	unsigned long int	iWIDTH_bytes= 	 nX*sizeof(uint8_T  );
	unsigned long int	oWIDTH_bytes= 	 nX*sizeof(uint32_T );
	unsigned long int	iMaps_bytes = nY*nX*sizeof(uint8_T  );
	unsigned long int 	oMap_bytes  = nY*nX*sizeof(uint32_T );
	unsigned long int 	chMat_bytes = nclasses*nclasses*nY*sizeof(uint32_T);
...
/*	tiledimX	= 128;
	tiledimY	= 128;
	if( nX%tiledimX ) nX = nX + (tiledimX - nX%tiledimX);
	if( nY%tiledimY ) nY = nY + (tiledimY - nY%tiledimY);
	if( nX%tiledimX ) deltaX = 1;
	if( nY%tiledimY ) deltaY = 1;
	ntilesX		= nX/tiledimX + deltaX;
	ntilesY		= nY/tiledimY + deltaY;
*/
	//
...
//	if(iPitch1 != iPitch2){ printf("Error: the pitch of the two input iMap's are different!\n"); return 1; }
	...
	*/
	...
	changemap<<<grid,block>>>(	dev_iMap1,dev_iMap2,
	...
	for(int i=0;i<nclasses;i++){
		for(int j=0;j<nclasses;j++){
			fprintf(fid, "%8d ", host_chMat[i+j*nclasses]);
		}fprintf(fid,"\n");
	} fclose(fid);
